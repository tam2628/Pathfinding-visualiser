...
import { GridFactory, Pair, Grid } from "./Grid";
import { GridNode } from "./Node";
import { dfs } from "./algos/dfs";
import { dijkstra } from "./algos/dijkstras";

export type State = {
  mouseDown: boolean;
};

export function StateEventManager() {
  const subs: any = [];
  let state: State = {
    mouseDown: false,
  };
...
export const mouseDownEventManager = StateEventManager();
...
window.onload = () => {
  ...
  algos_select?.addEventListener("change", (e) => {
    ...
    (
      visualiseBtn as HTMLButtonElement
    ).innerText = `Visualize ${SELECTED_ALGO}`;
  });
};
